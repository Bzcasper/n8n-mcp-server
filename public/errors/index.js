 ...
 */
function formatErrorMessage(message, statusCode, details) {
 ...
 */
export function handleAxiosError(error, defaultMessage = "n8n API request failed") {
    // Handle axios timeout errors specifically
    if (error.code === "ECONNABORTED" && error.message?.includes("timeout")) {
        return new N8nApiError(`Request timeout: ${defaultMessage}`, 408, // Request Timeout HTTP status
        { timeout: true, code: error.code });
    }
    ...
    // Handle request errors (e.g., network issues, connection refused)
    if (error.request) {
        return new N8nApiError("Network error connecting to n8n API", undefined, error.message || { code: error.code });
    }
    // Handle other errors
