...
export function loadEnvironmentVariables() {
    const { N8N_API_URL, N8N_API_KEY, N8N_WEBHOOK_BASE_URL, N8N_WEBHOOK_USERNAME, N8N_WEBHOOK_PASSWORD, } = process.env;
    if (!N8N_API_URL &&
        !N8N_API_KEY &&
...
export function getEnvConfig() {
    ...
    const mcpServerTimeout = parseInt(process.env[ENV_VARS.MCP_SERVER_TIMEOUT] || "30000", 10);
    ...
    if (!n8nApiUrl) {
        throw new McpError(ErrorCode.InitializationError, `Missing required environment variable: ${ENV_VARS.N8N_API_URL}`);
    }
    if (!n8nApiKey) {
        throw new McpError(ErrorCode.InitializationError, `Missing required environment variable: ${ENV_VARS.N8N_API_KEY}`);
    }
    ...
    try {
        ...
        throw new McpError(ErrorCode.InitializationError, `Invalid URL format for ${ENV_VARS.N8N_API_URL}: ${n8nApiUrl}`);
    }
    ...
    if (n8nWebhookBaseUrl) {
        try {
            ...
            throw new McpError(ErrorCode.InitializationError, `Invalid URL format for ${ENV_VARS.N8N_WEBHOOK_BASE_URL}: ${n8nWebhookBaseUrl}`);
        }
    }
    // Validate timeouts are positive numbers
    if (isNaN(n8nApiTimeout) || n8nApiTimeout <= 0) {
        throw new McpError(ErrorCode.InitializationError, `Invalid timeout for ${ENV_VARS.N8N_API_TIMEOUT}: must be a positive number, got ${n8nApiTimeout}`);
    }
    if (isNaN(mcpServerTimeout) || mcpServerTimeout <= 0) {
        throw new McpError(ErrorCode.InitializationError, `Invalid timeout for ${ENV_VARS.MCP_SERVER_TIMEOUT}: must be a positive number, got ${mcpServerTimeout}`);
    }
    ...
    if (corsAllowedOrigins && corsAllowedOrigins.trim() !== "") {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InitializationError, `Invalid format for ${ENV_VARS.CORS_ALLOWED_ORIGINS}: must be comma-separated URLs or '*', got ${corsAllowedOrigins}`);
        }
    }
    ...
    if (redisUrl) {
        try {
            ...
            throw new McpError(ErrorCode.InitializationError, `Invalid URL format for ${ENV_VARS.REDIS_URL}: ${redisUrl}`);
        }
    }
    ...
    if (postgresUrl) {
        try {
            ...
            throw new McpError(ErrorCode.InitializationError, `Invalid URL format for ${ENV_VARS.POSTGRES_URL}: ${postgresUrl}`);
        }
    }
    ...
    if (supabaseUrl) {
        try {
            ...
            throw new McpError(ErrorCode.InitializationError, `Invalid URL format for ${ENV_VARS.SUPABASE_URL}: ${supabaseUrl}`);
        }
    }
    ...
    if (supabaseAnonKey &&
        ...
        throw new McpError(ErrorCode.InitializationError, `Invalid format for ${ENV_VARS.SUPABASE_ANON_KEY}: should be a valid JWT token starting with 'eyJ'`);
    }
    if (supabaseServiceRoleKey &&
        (supabaseServiceRoleKey.length < 100 ||
            !supabaseServiceRoleKey.startsWith("eyJ"))) {
        throw new McpError(ErrorCode.InitializationError, `Invalid format for ${ENV_VARS.SUPABASE_SERVICE_ROLE_KEY}: should be a valid JWT token starting with 'eyJ'`);
    }
    // Validate database timeout
    if (isNaN(databaseTimeout) || databaseTimeout <= 0) {
        throw new McpError(ErrorCode.InitializationError, `Invalid database timeout for ${ENV_VARS.DATABASE_TIMEOUT}: must be a positive number, got ${databaseTimeout}`);
    }
    // Validate database max connections
    if (isNaN(databaseMaxConnections) ||
        databaseMaxConnections <= 0 ||
        databaseMaxConnections > 100) {
        throw new McpError(ErrorCode.InitializationError, `Invalid database max connections for ${ENV_VARS.DATABASE_MAX_CONNECTIONS}: must be between 1-100, got ${databaseMaxConnections}`);
    }
    return {
        n8nApiUrl,
        n8nApiKey,
