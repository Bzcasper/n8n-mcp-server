...
import { getWorkflowsResource, getWorkflowsResourceMetadata, getWorkflowsResourceUri, } from "./static/workflows.js";
import { getExecutionStatsResource, getExecutionStatsResourceMetadata, getExecutionStatsResourceUri, } from "./static/execution-stats.js";
// Import dynamic resource handlers
import { getWorkflowResource, getWorkflowResourceTemplateMetadata, extractWorkflowIdFromUri, } from "./dynamic/workflow.js";
import { getExecutionResource, getExecutionResourceTemplateMetadata, extractExecutionIdFromUri, } from "./dynamic/execution.js";
import { getCredentialResource, getCredentialResourceTemplateMetadata, extractCredentialIdFromUri, } from "./dynamic/credential.js";
/**
 ...
 */
export function setupResourceHandlers(server, envConfig) {
 ...
 * @param envConfig Environment configuration
 */
function setupStaticResources(server, _envConfig) {
 ...
 * @param envConfig Environment configuration
 */
function setupDynamicResources(server, envConfig) {
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        try {
            ...
            trackResourceAccess(uri.includes("workflows")
                ? "workflows"
                : uri.includes("executions")
                    ? "executions"
                    : uri.includes("credentials")
                        ? "credentials"
                        : "resources", false, duration);
            // Pass through McpErrors
            if (error instanceof McpError) {
                throw error;
            }
            // Convert other errors to McpError
            throw new McpError(ErrorCode.InternalError, `Error retrieving resource: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    });
}
