...
export class N8nApiClient {
    ...
    constructor(config) {
        ...
        if (config.debug) {
            this.axiosInstance.interceptors.request.use((request) => {
                console.error(`[DEBUG] Request: ${request.method?.toUpperCase()} ${request.baseURL}${request.url}`);
                return request;
            });
            this.axiosInstance.interceptors.response.use((response) => {
                ...
                console.error(`[DEBUG] Response Error: ${error.response?.status} ${error.response?.statusText}`, error.message);
                throw error;
            });
        }
    }
    ...
    async checkConnectivity() {
        try {
            ...
            if (response.status !== 200) {
                throw new N8nApiError("n8n API connectivity check failed", response.status);
            }
    ...
    async transferWorkflow(id, destinationProjectId) {
        try {
            const response = await this.axiosInstance.put(`/workflows/${id}/transfer`, { destinationProjectId });
            return response.data;
        }
        catch (error) {
    ...
    async transferCredential(id, destinationProjectId) {
        try {
            const response = await this.axiosInstance.put(`/credentials/${id}/transfer`, { destinationProjectId });
            return response.data;
        }
        catch (error) {
            throw handleAxiosError(error, `Failed to transfer credential ${id} to project ${destinationProjectId}`);
        }
    }
    /**
    ...
    async getCredentialSchema(credentialTypeName) {
        try {
            const response = await this.axiosInstance.get(`/credentials/schema/${credentialTypeName}`);
            return response.data;
        }
        catch (error) {
            throw handleAxiosError(error, `Failed to fetch schema for credential type ${credentialTypeName}`);
        }
    }
    /**
     * Delete a credential
