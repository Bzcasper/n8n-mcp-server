...
export class GetWorkflowTagsHandler extends BaseWorkflowToolHandler {
    ...
    async execute(args) {
        return this.handleExecution(async (args) => {
            ...
            return this.formatSuccess(tags, `Retrieved ${tags.length} tags for workflow: ${id}`);
        }, args);
    }
}
/**
    ...
    async execute(args) {
        return this.handleExecution(async (args) => {
            ...
            if (!tagIds || !Array.isArray(tagIds)) {
                throw new N8nApiError("Missing required parameter: tagIds (array of tag IDs)");
            }
            ...
            if (tagIds.length > 0 && typeof tagIds[0] === "string") {
                ...
                const tags = await this.apiService.updateWorkflowTags(id, processedTagIds);
                return this.formatSuccess(tags, `Updated tags for workflow: ${id} (set to ${tagIds.length} tags)`);
            }
            else {
 ...
 */
export function getGetWorkflowTagsToolDefinition() {
    return {
 ...
 * @returns Tool definition
 */
export function getUpdateWorkflowTagsToolDefinition() {
    return {
