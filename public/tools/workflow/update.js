...
export class UpdateWorkflowHandler extends BaseWorkflowToolHandler {
    ...
    async execute(args) {
        return this.handleExecution(async (args) => {
            ...
            // Update the workflow
            const updatedWorkflow = await this.apiService.updateWorkflow(id, workflowData);
            // Build a summary of changes
            const changesArray = [];
            if (name !== undefined && name !== currentWorkflow.name)
                changesArray.push(`name: "${currentWorkflow.name}" → "${name}"`);
            if (nodes !== undefined)
                changesArray.push("nodes updated");
            if (connections !== undefined)
                changesArray.push("connections updated");
            // Add warnings for read-only properties
            const warnings = [];
            if (active !== undefined)
                warnings.push("active (read-only, use activate/deactivate workflow tools)");
            if (tags !== undefined)
                warnings.push("tags (read-only property)");
            const changesSummary = changesArray.length > 0
                ? `Changes: ${changesArray.join(", ")}`
                : "No changes were made";
            const warningsSummary = warnings.length > 0 ? ` Note: Ignored ${warnings.join(", ")}.` : "";
 ...
 */
export function getUpdateWorkflowToolDefinition() {
    return {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                active: {
                    type: "boolean",
                    description: "Workflow active status (read-only: use activate/deactivate workflow tools instead)",
                },
                tags: {
                    type: "array",
