...
export class SchedulingWorkflowHandler extends BaseWorkflowToolHandler {
    ...
    async execute(args) {
        return this.handleExecution(async () => {
            const { name, scheduleType, batchSize, targetWorkflows, scheduleExpression, maxConcurrency, failurePolicy, } = args;
            ...
            const createdWorkflow = await this.apiService.createWorkflow(scheduledWorkflow);
...
export class MonitoringWorkflowHandler extends BaseWorkflowToolHandler {
    ...
    async execute(args) {
        return this.handleExecution(async () => {
            const { name, targetWorkflows, metrics, alertThresholds, dashboardConfig, } = args;
            ...
            const createdWorkflow = await this.apiService.createWorkflow(monitoringWorkflow);
            return this.formatSuccess({
                id: createdWorkflow.id,
                name: createdWorkflow.name,
 ...
 */
export function getCreateScheduledWorkflowToolDefinition() {
    return {
        name: "n8n-workflow-create-scheduled",
        description: "Create a scheduled batch workflow for processing multiple workflows with specific timing",
        inputSchema: {
            type: "object",
            properties: {
                name: {
                ...
                scheduleExpression: {
                    type: "string",
                    description: "Cron expression for scheduling (e.g., '0 0 * * *' for daily)",
                },
                maxConcurrency: {
                    type: "integer",
                ...
                failurePolicy: {
                    type: "string",
                    description: "How to handle failures (continueOnFail, stopOnFail, retry)",
 ...
 */
export function getCreateMonitoringWorkflowToolDefinition() {
    return {
        name: "n8n-workflow-create-monitoring",
        description: "Create a monitoring and analytics workflow with alerts and dashboard updates",
        inputSchema: {
            type: "object",
            properties: {
                name: {
