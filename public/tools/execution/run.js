...
const runWebhookSchema = z.object({
    workflowName: z
        .string()
        .describe('Name of the workflow to execute (e.g., "hello-world")'),
    data: z
        .record(z.any())
        .optional()
        .describe("Input data to pass to the webhook"),
    headers: z
        .record(z.string())
        .optional()
...
export class RunWebhookHandler extends BaseExecutionToolHandler {
    ...
    getN8nBaseUrl() {
        ...
        // Use explicit webhook base URL if provided
        if (config.n8nWebhookBaseUrl) {
            return config.n8nWebhookBaseUrl;
        }
        // Otherwise derive from API URL by removing /api/v1
    ...
    async execute(args) {
        return this.handleExecution(async (args) => {
            ...
            if (!config.n8nWebhookUsername || !config.n8nWebhookPassword) {
                throw new N8nApiError("Webhook username and password are required for run_webhook tool. " +
                    "Please set N8N_WEBHOOK_USERNAME and N8N_WEBHOOK_PASSWORD environment variables.", 400 // Bad Request, as it's a client-side configuration issue for this specific tool
                );
            }
            try {
                ...
                const response = await axios.post(webhookUrl, params.data || {}, requestConfig);
                ...
                if (axios.isAxiosError(error)) {
                    ...
                    if (error.response) {
                        ...
                        if (error.response.data) {
                            return this.formatError(new N8nApiError(`${errorMessage}\n\n${JSON.stringify(error.response.data, null, 2)}`, error.response.status));
                        }
                    }
 ...
 */
RunWebhookHandler.inputSchema = runWebhookSchema;
/**
