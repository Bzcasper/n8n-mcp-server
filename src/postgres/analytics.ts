 ...
 */
export async function getWorkflowPerformanceSummary(
  limit: number = 50,
  offset: number = 0
...
export async function getExecutionTrends(
  ...
  return result.rows.map((row) => ({
    ...
    averageDuration: Number(row.avg_duration) || 0,
    trendDirection: row.trend_direction as "up" | "down" | "stable",
  }));
}
 ...
 */
export async function getPerformanceMetricsByType(days: number = 7): Promise<
  Array<{
    metricType: string;
    workflowId: string;
    workflowName: string;
...
export async function getSystemHealthOverview(): Promise<{
  ...
  const workflowRow =
    (workflowStats as any).rows?.[0] || (workflowStats as any)[0] || {};
  const executionRow =
    (executionStats as any).rows?.[0] || (executionStats as any)[0] || {};
  const userRow = (userStats as any).rows?.[0] || (userStats as any)[0] || {};
 ...
 */
export async function updateProductivityScores(): Promise<void> {
  await sql`
