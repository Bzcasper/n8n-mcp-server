...
export class HybridCacheManager {
  ...
  private async validateConnections(): Promise<void> {
    ...
    const kvHealthy =
      (await this.kvCache.get("__health_check")) !== null || true; // Assume KV is available unless proven otherwise

    if (!postgresHealthy && !kvHealthy) {
      throw new McpError(
        ErrorCode.InitializationError,
        "Both PostgreSQL and KV cache are unavailable"
      );
    }
  ...
  async get<T>(key: string, fetchFunction: () => Promise<T>): Promise<T> {
    ...
    // Check KV cache
    const kvResult = await this.kvCache.getJson<T>(key);
    if (kvResult !== null) {
    ...
    try {
      ...
      if (this.config.fallbackEnabled) {
        console.warn(
          `Failed to fetch ${key}, attempting cached recovery`,
          error
        );
...
export class WorkflowDataAccess {
  ...
  async getWorkflows(limit: number = 50, offset: number = 0): Promise<any[]> {
  ...
  async updateWorkflow(id: string, updates: any): Promise<void> {
    try {
      const setStatements = [];
      const values = [];
  ...
  private async invalidateWorkflowCaches(): Promise<void> {
...
export class ExecutionDataAccess {
  ...
  async getExecutions(workflowId?: string, limit: number = 50): Promise<any[]> {
  ...
  async createExecution(executionData: any): Promise<string> {
...
export class AnalyticsDataAccess {
  private cache: HybridCacheManager;
  private cacheExpiry = 3600; // 1 hour for analytics data
  ...
  async getWorkflowAnalytics(days: number = 30): Promise<any[]> {
  ...
  async getSystemOverview(): Promise<any> {
    ...
    return this.cache.get(cacheKey, async () => {
      ...
      return {
        totalWorkflows:
          Number((workflowStats.rows || workflowStats)[0]?.count) || 0,
        totalUsers: Number((userStats.rows || userStats)[0]?.count) || 0,
        totalExecutions24h:
          Number(
            (executionStats.rows || executionStats)[0]?.total_executions
          ) || 0,
        avgExecutionTime24h:
          Number(
            (executionStats.rows || executionStats)[0]?.avg_execution_time
          ) || 0,
        successRate24h:
          (Number(
            (executionStats.rows || executionStats)[0]?.successful_executions
          ) /
            Math.max(
              1,
              Number(
 ...
 * Graceful shutdown
 */
export async function shutdownHybridStorage(): Promise<void> {
