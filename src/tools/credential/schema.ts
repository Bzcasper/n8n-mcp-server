...
export class GetCredentialSchemaHandler extends BaseCredentialToolHandler {
  ...
  async execute(args: Record<string, any>): Promise<ToolCallResult> {
    return this.handleExecution(async (args) => {
      ...
      const schema = await this.apiService.getCredentialSchema(
        credentialTypeName
      );

      return this.formatSuccess(
        schema,
        `Retrieved credential schema for type: ${credentialTypeName}`
      );
    }, args);
  }
}
 ...
 */
export function getGetCredentialSchemaToolDefinition(): ToolDefinition {
  return {
