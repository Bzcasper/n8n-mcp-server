...
export class TransferCredentialHandler extends BaseCredentialToolHandler {
  ...
  async execute(args: Record<string, any>): Promise<ToolCallResult> {
    return this.handleExecution(async (args) => {
      ...
      const transferredCredential = await this.apiService.transferCredential(
        id,
        destinationProjectId
      );

      return this.formatSuccess(
        { id, destinationProjectId, transferredCredential },
        `Credential "${credentialName}" (ID: ${id}) has been successfully transferred to project ${destinationProjectId}`
      );
    }, args);
  }
}
 ...
 */
export function getTransferCredentialToolDefinition(): ToolDefinition {
  return {
